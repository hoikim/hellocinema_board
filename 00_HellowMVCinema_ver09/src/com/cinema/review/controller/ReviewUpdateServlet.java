...
public class ReviewUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int starScore = Integer.parseInt(request.getParameter("star"));
		
		Review rv = new Review();
		rv.setRvid(rvid);
		rv.setContent(content);
		rv.setStarScore(starScore);
...
//					int totalPage = (Integer)(totalMember / numPerPage) + (totalMember % numPerPage); //팀장님.
		...
		System.out.println("list@ReviewUpdateServlet="+list);
		...
		while(pageNo <= pageEnd && pageNo <=totalPage) {
			if(pageNo==cPage) {
				...
				//pageBar +="<a href="+request.getContextPath()+"/review/reviewlist.do?cPage="+pageNo+"&mid="+mid+"><span>"+pageNo+"</span></a>";
