...
<h2>고객센터</h2>
	...
	<div id="div-notice-enroll" class='div-enroll'>
		...
		<form action="<%=request.getContextPath()%>/board/boardEnroll" method='post' enctype="multipart/form-data">
			...
			<br />
			<div id="theater-div"></div>	
			<br />
			이미지 : <input type="file" name="up_notice_file" />
			<br />
			내용 : <textarea name="content" id="img2" cols="20" rows="10"></textarea>
			<br />
			<br />
			...
			$(function(){
				...
				$("#local1").change(function(){
					...
					if(localId!="전체"){
						...
						$("#theater-div").html("<input type='hidden' name='local' value='전체' />");
					}
					});	//#local1 change
				...
				$.ajax({
					...
					success : function(data){
						...
						$(".close").click(function(){
							...
							$(".div-enroll input:submit").val("등록");
							$(".div-enroll input:reset").val("취소");
						});
	...
	<div id="div-event-enroll" class='div-enroll'>
		...
		<form action="<%=request.getContextPath()%>/board/boardEnroll"  method='post' enctype="multipart/form-data">
			...
			<br />
			내용 : <textarea name="content" id="img1" cols="20" rows="10"></textarea>
			<br />
			시작:
	...
	<script>
		...
		$("#eventList").click(function(){
			...
			$("#eventTime").click(function(){
				...
				//2018-5-12 ~ 2018-4-18과 같이 시간을 잘못 설정할 경우//2018*(월/12)의 값이 시작시간이 클 경우
				if( (startterm[0]*1+startterm[1]/12)  > (endterm[0]*1+endterm[1]/12) ){	
					...
					$("div#none").on("click","table.table", function(){
						...
						for(var a in data){
							...
							if(b.bdid==this.id){
								...
								html += "<p class='update-event'>제목 : </p><input type=date class='update-event' name='startterm' id='update-start-pst' value='"+dsYear + "-" + dsMonth + "-" + dsDate + "'><p class='update-event'>~</p><input type=date class='update-event' name='endterm' id='update-end-pst' value='"  + deYear + "-" + deMonth + "-" + deDate+"' ><br class='update-event'/>";
								html += "<input type='button' value='수정' class='event-popup' ><hr class='event-popup'><br>";
								html += "<input type='submit' value='수정' class='update-event' id='update-submit-pst'><hr class='update-event'><br>";
						...
						//이벤트 수정 시작시간과 끝시간의 잘못된 설정을 막아준다.
						$("#event-popup").on("click","#update-submit-pst",function(){
							console.log("클릭클릭");
							var startterm = $("#update-start-pst").val().split("-");
							var endterm = $("#update-end-pst").val().split("-");
					...
					$("div#none").on("click","table.table", function(){
						...
						for(var a in data){
							...
							if(b.bdid==this.id){
								...
								html += "<p class='update-event'>제목 : </p><input type='text' class='update-event' name='title' value='"+b.title+"'/>" ;
								html += "<p class='event-popup'>"+dsYear + "-" + dsMonth + "-" + dsDate + " ~ "  + deYear + "-" + deMonth + "-" + deDate+"</p><br/>";
								html += "<p class='update-event'>제목 : </p><input type=date class='update-event' name='startterm' id='update-start' value='"+dsYear + "-" + dsMonth + "-" + dsDate + "'><p class='update-event'>~</p><input type=date class='update-event' name='endterm' id='update-end' value='"  + deYear + "-" + deMonth + "-" + deDate+"' ><br class='update-event'/>";
								html += "<input type='button' value='수정' class='event-popup' ><hr class='event-popup'><br>";
								html += "<input type='submit' value='수정' class='update-event' id='update-submit'><hr class='update-event'><br>";
								html += "<img src='<%=request.getContextPath()%>/upload/board/"+ b.img +"'/ class='event-popup'>";								
								html += "<input type='file' name='up-notice-file' class='update-event' />";								
								html += "</form>";
							}
						...
						//이벤트 수정 시작시간과 끝시간의 잘못된 설정을 막아준다.
						$("#event-popup").on("click","#update-submit",function(){
							console.log("클릭클릭");
							var startterm = $("#update-start").val().split("-");
							var endterm = $("#update-end").val().split("-");
							console.log(startterm);
							console.log(endterm);
							//2018-5-12 ~ 2018-4-18과 같이 시간을 잘못 설정할 경우
							if(startterm[0]*1+startterm[1]/12>endterm[0]*1+endterm[1]/12){	//2018*(월/12)의 값이 시작시간이 클 경우
